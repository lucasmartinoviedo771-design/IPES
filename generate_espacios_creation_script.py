...
with open(
    r"C:\proyectos\academia\parsed_correlatividades.json", "r", encoding="utf-8"
) as f:
    data = json.load(f)
...
for year_data in data["years"]:
    ...
    if anio:
        for espacio_data in year_data["espacios"]:
            ...
            cuatrimestre = (
                "A"  # Default to Anual, needs to be confirmed with user or data
            )
...
print(
    f"# Missing EspacioCurricular objects for {profesorado_name} (Plan {plan_resolucion})"
)
print(f"# Run this in Django shell or a management command")
print(
    f"# from academia_core.models import Profesorado, PlanEstudios, EspacioCurricular"
)
print(f"# prof = Profesorado.objects.get(nombre='{profesorado_name}')")
print(
    f"# plan = PlanEstudios.objects.get(profesorado=prof, resolucion='{plan_resolucion}')"
)
...
for esp in unique_espacios:
    ...
    print(
        f"    defaults={{ 'horas': 0, 'formato': '{esp['format']}', 'libre_habilitado': False }}"
    )
    print(f")")
