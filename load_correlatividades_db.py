...
for year_data in data["years"]:
    ...
    if anio_num_match:
        ...
        # Add more years as needed
            ...
            print(
                f"Warning: Unknown year name '{year_name}'. Skipping spaces for this year."
            )
            continue
    else:
        print(
            f"Warning: Could not parse year name '{year_name}'. Skipping spaces for this year."
        )
        continue
    ...
    for espacio_data in year_data["espacios"]:
        ...
        for req_aprobada in espacio_data["correlativas_cursar"]["aprobadas"]:
            ...
            req_espacio_name_for_query = re.sub(
                r"\s*\([A-Z]\)$", "", req_espacio_name
            ).strip()
