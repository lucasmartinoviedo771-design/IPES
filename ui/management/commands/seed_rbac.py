...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        groups = {
            name: Group.objects.get_or_create(name=name)[0] for name in GROUPS.keys()
        }
    ...
    def _apply(self, group: Group, desired_codenames: list[str]):
        ...
        for code in desired_codenames:
            ...
            if not qs.exists():
                self.stdout.write(
                    self.style.WARNING(f"Permiso no encontrado (codename): {code}")
                )
                continue
            for p in qs:
                resolved.append(p)
                self.stdout.write(
                    f"  OK {group.name} + {p.content_type.app_label}.{p.codename}"
                )
        group.permissions.set(resolved)
        self.stdout.write(
            self.style.SUCCESS(f"Asignados {len(resolved)} permisos a {group.name}")
        )
