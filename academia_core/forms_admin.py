...
# === Eliminar ==============================
class DeleteProfesoradoForm(forms.Form):
    profesorado = forms.ModelChoiceField(
        queryset=Profesorado.objects.all().order_by("nombre"),
        label="Profesorado"
    )
    def delete_object(self):
        obj = self.cleaned_data["profesorado"]
        obj.delete()
        return obj
...
class DeletePlanForm(forms.Form):
    ...
    def delete_object(self):
        obj = self.cleaned_data["plan"]
        obj.delete()
        return obj

class DeleteEspacioForm(forms.Form):
    profesorado = forms.ModelChoiceField(
        queryset=Profesorado.objects.all().order_by("nombre"),
        label="Profesorado"
    )
    ...
    def delete_object(self):
        obj = self.cleaned_data["espacio"]
        obj.delete()
        return obj

# === Alias en español para compatibilidad con tus vistas =========
RenombrarProfesoradoForm = RenameProfesoradoForm
RenombrarPlanForm        = RenamePlanForm
RenombrarEspacioForm     = RenameEspacioForm

EliminarProfesoradoForm  = DeleteProfesoradoForm
EliminarPlanForm         = DeletePlanForm
EliminarEspacioForm      = DeleteEspacioForm

ProfesoradoForm          = ProfesoradoCreateForm
PlanForm                 = PlanCreateForm

__all__ = [
    # crear
    "ProfesoradoCreateForm", "PlanCreateForm",
    "ProfesoradoForm", "PlanForm",
    # renombrar
    "RenameProfesoradoForm", "RenamePlanForm", "RenameEspacioForm",
    "RenombrarProfesoradoForm", "RenombrarPlanForm", "RenombrarEspacioForm",
    # eliminar
    "DeleteProfesoradoForm", "DeletePlanForm", "DeleteEspacioForm",
    "EliminarProfesoradoForm", "EliminarPlanForm", "EliminarEspacioForm",
]
