...
from .models import (
    ...
    EspacioCurricular, Movimiento, Docente, DocenteEspacio
)
...
def _ensure_slug_attrs(prof: Profesorado, plan: PlanEstudios):
    ...
    try:
        plan.resolucion_slug = getattr(plan, "resolucion_slug", (plan.resolucion or "").replace("/", "-"))
    except Exception:
...
def _build_carton_ctx_base(prof, plan, dni: str):
    ...
    for e in espacios:
        ...
        movs.sort(key=lambda m: (
            m.fecha or date.min,
            0 if m.tipo == "REG" else 1,
            m.id
        ))
...
def alumno_home(request):
    ...
    inscs = (EstudianteProfesorado.objects
             .filter(estudiante=est)
             .select_related("profesorado"))

    items = []
    for ins in inscs:
        plan = (PlanEstudios.objects
                .filter(profesorado=ins.profesorado, vigente=True).first()
        ...
        if plan:
            ...
            espacios = (EspacioCurricular.objects
                        .filter(profesorado=ins.profesorado, plan=plan)
                        .order_by("anio", "cuatrimestre", "nombre"))
            ...
            for e in espacios:
                ...
                if estado == "Aprobada":
                    ...
                    pendientes_list.append({
                        "anio": e.anio, "cuatri": e.cuatrimestre, "espacio": e.nombre, "ultimo": ult or "—",
                    })

                # Lista completa (para la tabla principal)
                todas.append({
                    "anio": e.anio, "cuatri": e.cuatrimestre, "espacio": e.nombre,
                    "estado": estado, "ultimo": ult or "—",
                })

        items.append({
            "ins": ins,
            "plan": plan,
            "cuentas": {"aprobadas": aprobadas, "desaprobadas": desaprobadas, "pendientes": pendientes},
...
def docente_espacio_detalle(request, espacio_id: int):
    ...
    de = get_object_or_404(DocenteEspacio, docente=perfil.docente, espacio_id=espacio_id)
    esp = de.espacio
    ...
    for insc, movs in alumnos:
        last = movs[-1] if movs else None
        if any(_es_aprobada(m) for m in movs):
            estado = "Aprobada"; aprob += 1
        elif last and _es_desaprobada(last):
            estado = "Desaprobada"; desa += 1
        else:
            estado = "Pendiente"; pend += 1
    ...
    filas.sort(key=lambda r: (order_key.get(r["estado"], 9), r["apellido"], r["nombre"]))

    ctx = {
        "docente": perfil.docente,
        "espacio": esp,
        "profesorado": prof,
        "resumen": {"aprobadas": aprob, "desaprobadas": desa, "pendientes": pend, "total": len(filas)},
        "filas": filas,
        "q": q,
    }
