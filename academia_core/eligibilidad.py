...
InscripcionEspacio = _first_model(["InscripcionEspacio", "InscripcionCursada", "InscripcionMateria"])
# Inscripción a final (opcional)
InscripcionFinal = _first_model(["InscripcionFinal", "MesaInscripcion"])

# ---------- estado académico sets ----------
def estado_sets_para_estudiante(estudiante_id: int, plan_id: int, ciclo: Optional[int] = None
    ...
    if ResultadoFinal:
        ...
        if fk_est and fk_esp:
            ...
            if fk_plan:
                qs = qs.filter(**{f"{fk_plan}_id" if not fk_plan.endswith("_id") else fk_plan: plan_id})
...
def _cumple_correlatividad(c: Correlatividad, aprob: Set[int], regs: Set[int], plan_id: int) -> bool:
    ...
    if c.requiere_todos_hasta_anio:
        ...
        ids_hasta = set(EspacioCurricular.objects.filter(plan_id=plan_id, anio__lte=hasta)
                        .values_list("id", flat=True))
        return ids_hasta.issubset(objetivo)

    return True

def habilitado(estudiante_id: int, plan_id: int, espacio: EspacioCurricular,
               para: str = "PARA_CURSAR", ciclo: Optional[int] = None) -> Tuple[bool, Any]:
    ...
    for c in correlativas_para(espacio.id, plan_id, para):
        if not _cumple_correlatividad(c, aprob, regs, plan_id):
            if c.requiere_espacio_id:
                faltantes.append({"tipo": (c.tipo or para).upper(),
