...
from academia_core.models import (
    Estudiante, Profesorado, PlanEstudios, EstudianteProfesorado,
    EspacioCurricular, Movimiento, Condicion, InscripcionEspacio, Docente, UserProfile
)
...
class EstudianteProfesoradoModelTest(TestCase):
    def setUp(self):
        ...
        self.plan1_prof1 = PlanEstudios.objects.create(profesorado=self.profesorado1, resolucion="Res. 001/2020")
        self.plan2_prof1 = PlanEstudios.objects.create(profesorado=self.profesorado1, resolucion="Res. 002/2020")
        self.plan1_prof2 = PlanEstudios.objects.create(profesorado=self.profesorado2, resolucion="Res. 003/2020")
    ...
    def test_clean_method_invalid_plan(self):
        ...
        with self.assertRaisesRegex(ValidationError, "El plan seleccionado no pertenece al profesorado."):
            inscripcion.full_clean()
...
class EspacioCurricularModelTest(TestCase):
    def setUp(self):
        ...
        self.plan = PlanEstudios.objects.create(profesorado=self.profesorado, resolucion="Res. 001/2020")
    ...
    def test_unique_espacio_plan_nombre_constraint(self):
        ...
        with self.assertRaises(IntegrityError):
            EspacioCurricular.objects.create(
                plan=self.plan, nombre="Matemática I", anio="2°", cuatrimestre="1" # Mismo plan y nombre
            )
...
class MovimientoModelTest(TestCase):
    def setUp(self):
        ...
        self.plan = PlanEstudios.objects.create(profesorado=self.profesorado, resolucion="Res. Test")
        self.inscripcion = EstudianteProfesorado.objects.create(
            estudiante=self.estudiante, profesorado=self.profesorado, plan=self.plan, cohorte=2023
        )
        self.espacio = EspacioCurricular.objects.create(
            plan=self.plan, nombre="Materia Test", anio="1°", cuatrimestre="1"
        )
        self.condicion_regular = Condicion.objects.create(codigo="REGULAR", nombre="Regular", tipo="REG")
        self.condicion_aprobado = Condicion.objects.create(codigo="APROBADO", nombre="Aprobado", tipo="REG")
        self.condicion_final_aprobado = Condicion.objects.create(codigo="REGULAR", nombre="Aprobado Final", tipo="FIN")

    def test_clean_method_valid_movimiento(self):
        movimiento = Movimiento(
            inscripcion=self.inscripcion, espacio=self.espacio, tipo="REG",
            fecha="2023-03-15", condicion=self.condicion_regular, nota_num=7.0
        )
        try:
    ...
    def test_clean_method_invalid_nota_regular(self):
        movimiento = Movimiento(
            inscripcion=self.inscripcion, espacio=self.espacio, tipo="REG",
            fecha="2023-03-15", condicion=self.condicion_regular, nota_num=11.0 # Nota inválida
        )
        with self.assertRaisesRegex(ValidationError, "La nota de Regularidad debe estar entre 0 y 10."):
            movimiento.full_clean()
    ...
    def test_clean_method_condicional_promocion(self):
        ...
        movimiento = Movimiento(
            inscripcion=self.inscripcion, espacio=self.espacio, tipo="REG",
            fecha="2023-03-15", condicion=self.condicion_aprobado, nota_num=8.0
        )
        with self.assertRaisesRegex(ValidationError, "Estudiante condicional: no puede quedar Aprobado/Promoción por cursada."):
            movimiento.full_clean()

    def test_clean_method_final_nota_minima(self):
        movimiento = Movimiento(
            inscripcion=self.inscripcion, espacio=self.espacio, tipo="FIN",
            fecha="2023-07-20", condicion=self.condicion_final_aprobado, nota_num=5.0 # Nota menor a 6
        )
        with self.assertRaisesRegex(ValidationError, "Nota de Final por regularidad debe ser >= 6."):
            movimiento.full_clean()
    ...
    def test_clean_method_espacio_profesorado_mismatch(self):
        ...
        otro_plan = PlanEstudios.objects.create(profesorado=otro_profesorado, resolucion="Res. Otro")
        otro_espacio = EspacioCurricular.objects.create(
            plan=otro_plan, nombre="Otra Materia", anio="1°", cuatrimestre="1"
        )
        movimiento = Movimiento(
            inscripcion=self.inscripcion, espacio=otro_espacio, tipo="REG",
            fecha="2023-03-15", condicion=self.condicion_regular, nota_num=7.0
        )
        with self.assertRaisesRegex(ValidationError, "El espacio no pertenece al mismo profesorado de la inscripción del estudiante."):
            movimiento.full_clean()
