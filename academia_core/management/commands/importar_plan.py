...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--csv", required=True, help="Ruta al CSV con anio,cuatrimestre,formato,nombre,horas")
    ...
    def handle(self, *args, **opts):
        ...
        plan, _ = PlanEstudios.objects.get_or_create(profesorado=p, resolucion=resol, defaults={"vigente": True})

        requeridos = {"anio","cuatrimestre","formato","nombre","horas"}
        mapa_cuatri = {
            "A":"A","ANUAL":"A",
            "1":"1","1º":"1","1°":"1","PRIMERO":"1",
            "2":"2","2º":"2","2°":"2","SEGUNDO":"2",
        }
        ...
        with open(csv_path, newline="", encoding="utf-8-sig") as f:  # acepta UTF-8 y UTF-8 con BOM
            ...
            if set(rdr.fieldnames or []) != requeridos:
                raise CommandError(f"Encabezados inválidos. Deben ser: {','.join(sorted(requeridos))}")
            ...
            for i, row in enumerate(rdr, start=2):
                ...
                _, created = EspacioCurricular.objects.get_or_create(
                    profesorado=p, plan=plan, anio=anio, cuatrimestre=cuatri, nombre=nombre,
