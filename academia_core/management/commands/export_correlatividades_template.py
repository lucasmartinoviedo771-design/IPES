...
HEADER = [
    "profesorado_slug", "plan_resolucion",
    "anio", "cuatrimestre", "espacio_nombre",
    "tipo", "requisito",
    "requiere_todos_hasta_anio",
    "requiere_espacio_anio", "requiere_espacio_cuatr", "requiere_espacio_nombre",
    "observaciones",
]
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--profesorado", required=True, help="slug del profesorado (p.ej. profesorado-de-educacion-primaria)")
    ...
    def handle(self, *args, **opts):
        ...
        espacios = EspacioCurricular.objects.filter(profesorado=prof, plan=plan).order_by("anio", "cuatrimestre", "nombre")
        ...
        with open(out, "w", newline="", encoding="utf-8") as f:
            ...
            for e in espacios:
                ...
                for tipo in ("CURSAR", "RENDIR"):
                    w.writerow([
                        prof.slug, plan.resolucion,
                        e.anio, e.cuatrimestre, e.nombre,
                        tipo, "",   # requisito: APROBADA / REGULARIZADA (completar)
        ...
        self.stdout.write(self.style.SUCCESS(f"Plantilla exportada a {out} ({espacios.count()} espacios x 2 filas)"))
